...
from typing import Literal, TypeVar, get_args, Union, get_origin
from types import UnionType
from dataclasses import is_dataclass
from ..typed.bnf import RuleSet, Rule
from ..typed.typetools import AnnotationReader
import json
import hashlib
...
class JsonFactory(BNFFactory):
    ...
    def compile(self, rules: RuleSet, clazz: type) -> tuple[str, dict[str, str]]:
        make = lambda f: lambda a, b, c: f(b)

        def handle_default(reader: AnnotationReader, rules: RuleSet, type: type) -> Rule:
            raise NotImplementedError("Not implemented yet.")

        def handle_list(reader: AnnotationReader, rules: RuleSet, type: type) -> Rule:
            if len(args := get_args(type)) != 1:
                raise TypeError("Unable to determine list type.")
            list_type = args[0]
            return JsonFactory.array(rules, reader.handle(rules, list_type))

        def handle_tuple(reader: AnnotationReader, rules: RuleSet, type: type) -> Rule:
            ...
            return JsonFactory.items(rules, *map(lambda t: reader.handle(rules, t), args))

        def handle_enum(reader: AnnotationReader, rules: RuleSet, type: type) -> Rule:
            if not (args := get_args(type)):
                raise TypeError("Unable to determine enum types.")

            if any(get_origin(x) is not Literal for x in args):
                raise TypeError("All items must be only either literals.")
            args = [get_args(arg)[0] for arg in args]

            return JsonFactory.enum(rules, *[rules.literal(x if not isinstance(x, str) else json.dumps(x)) for x in args])

        reader = AnnotationReader(
            {
                int: make(JsonFactory.number),
                float: make(JsonFactory.number),
                str: make(JsonFactory.string),
                bool: make(JsonFactory.boolean),
                list: handle_list,
                tuple: handle_tuple,
                Union: handle_enum,
        ...
        for k, v in clazz.__annotations__.items():
            obj[k] = reader.handle(rules, v)

        return JsonFactory.obj(rules, obj).rule_id, rules.declare()
    ...
    def deserialize(self, clazz: type[T], payload: str) -> T:
        ...
        return super().deserialize(clazz, payload)
